...
export class MyMenu extends LitElement {
  ...
  menuItems = [
    ...
    },
    {
      ...
      style:
        'display: list-item; margin-left: 1em; color: rgb(12, 11, 10); margin-left: 40px',
    },
  ];
    ...
    return html`
      <div class="wrapper">
        <div class="container">
          ...
          ${this.menuItems.map(
            (item, index) =>
              html` <div class="block" @click=${() => this._onClickMenu(index)}>
  ...
  private _onClickMenu(index: Number) {
    const mainInput =
      (this.parentElement?.getElementsByClassName(
        'main-input'
      ) as HTMLCollectionOf<Element>) || undefined;
